...
timeseries %>%
  ...
  labs(title = "Case counts",
       x = "Date",
       y = "Cases") +
  scale_x_date(date_breaks = "1 week", date_minor_breaks = "1 week", date_labels = "%d %b \n%Y") +
  whotools::theme_who()
...
The model estimates the effective reproduction number as `r model_ests$summary[3] %>% select(numeric_estimate) %>% unlist() %>% .['numeric_estimate.median']/100` (`r model_ests$summary[3] %>% select(numeric_estimate) %>% unlist() %>% .['numeric_estimate.lower_50']/100`, `r model_ests$summary[3] %>% select(numeric_estimate) %>% unlist() %>% .['numeric_estimate.upper_50']/100`). 


```{r, fig.cap = "Fig 2 Estimates of time varying reproduction number", fig.height=3.5, fig.width=6}
...
```{r, fig.cap = "Fig 3 Estimated reported cases", fig.height=3.5, fig.width=6}
