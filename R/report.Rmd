...
in the data collected and interpretation in real-time is challenging. This can be due to reporting delays, where cases in the current or recent weeks have not yet been fully reported due to
logistical challenges in the field, limited laboratory capacity or other factors. 
...
The model estimates the effective reproduction number as `r model_ests$summary[3] %>% select(numeric_estimate) %>% unlist() %>% .['numeric_estimate.median']` (`r model_ests$summary[3] %>% select(numeric_estimate) %>% unlist() %>% .['numeric_estimate.lower_50']`, `r model_ests$summary[3] %>% select(numeric_estimate) %>% unlist() %>% .['numeric_estimate.upper_50']`). 


```{r, fig.cap = "Fig 2 Estimates of time varying reproduction number"}
...
The number of cases in the next `r horizon` days is estimated to be `r model_ests$estimates$summarised %>% filter(variable == "reported_cases") %>% filter(type == "forecast") %>% summarize(sum(median))` (`r model_ests$estimates$summarised %>% filter(variable == "reported_cases") %>% filter(type == "forecast") %>% summarize(sum(lower_50))`, `r model_ests$estimates$summarised %>% filter(variable == "reported_cases") %>% filter(type == "forecast") %>% summarize(sum(upper_50))`). The trend is shown in Figure 3.


```{r, fig.cap = "Fig 3 Estimated reported cases"}

rep_ests <- model_ests$estimates$summarised %>% filter(variable == "reported_cases") 

viz_reported_week(model_ests, adm_names)
